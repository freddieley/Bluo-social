...
export function AvatarCreator({config,setConfig,close}:{config:AvatarConfig;setConfig:(c:AvatarConfig)=>void;close:()=>void}){
 ...
 return <div className="avatar-creator">
  <style jsx>{`
   ...
   .avatar-creator-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;padding:2px 2px 0}.avatar-creator-head h2{font-family:'Plus Jakarta Sans';font-size:24px;line-height:1.1;letter-spacing:-.8px;color:#0d2d63;margin:5px 0 6px}.avatar-creator-head p{margin:0;max-width:470px;color:#7183a3;font-size:12px;line-height:1.5}.avatar-creator-spark{width:40px;height:40px;border-radius:50%;background:#1479ff;color:#fff;display:grid;place-items:center;flex:none;box-shadow:0 7px 18px rgba(20,121,255,.2)}
   .avatar-creator-stage{display:grid;grid-template-columns:220px 1fr;gap:20px;align-items:center;padding:14px;border:1px solid #dceafa;border-radius:22px;background:linear-gradient(135deg,#edf7ff 0%,#fff 70%);box-shadow:0 10px 28px rgba(17,65,130,.07)}.avatar-creator-preview{min-height:190px;display:grid;place-items:center;border-radius:18px;background:rgba(255,255,255,.72)}.avatar-creator-preview :global(svg){filter:drop-shadow(0 10px 12px rgba(36,70,108,.12))}.avatar-creator-preview-copy{display:flex;flex-direction:column;gap:7px;padding-right:8px}.avatar-preview-kicker{font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:#1479ff}.avatar-creator-preview-copy b{font-family:'Plus Jakarta Sans';font-size:17px;line-height:1.2;color:#0d2d63}.avatar-creator-preview-copy span{font-size:12px;color:#7183a3;line-height:1.55;max-width:330px}
   .avatar-tabs{display:flex;gap:3px;overflow-x:auto;padding:4px;border:1px solid #dfe9f5;background:#f5f8fc;border-radius:15px;scrollbar-width:none}.avatar-tabs::-webkit-scrollbar{display:none}.avatar-tab{border:0;white-space:nowrap;padding:9px 12px;border-radius:11px;background:transparent;color:#7183a3;font-size:11px;font-weight:800;transition:.15s}.avatar-tab:hover{color:#0d2d63;background:#edf3fa}.avatar-tab.active{background:#fff;color:#1479ff;box-shadow:0 3px 10px rgba(17,65,130,.09)}
   .avatar-option-panel{min-height:175px}.avatar-choice-section{display:flex;flex-direction:column;gap:10px}.avatar-choice-title{font-size:12px;font-weight:800;color:#0d2d63}.avatar-choice-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.avatar-choice{position:relative;min-width:0;padding:7px 5px 8px;border:1px solid #e0eaf5;background:#fff;border-radius:16px;text-align:center;transition:transform .15s,border-color .15s,box-shadow .15s,background .15s}.avatar-choice:hover{transform:translateY(-1px);border-color:#a9cdfb;box-shadow:0 7px 16px rgba(20,121,255,.08)}.avatar-choice.selected{border-color:#6aaeff;background:#eef6ff;box-shadow:inset 0 0 0 1px #6aaeff,0 7px 18px rgba(20,121,255,.09)}.avatar-choice-art{display:grid;place-items:center;min-height:84px;border-radius:12px;background:#f8faff}.avatar-choice-art :global(svg){filter:drop-shadow(0 4px 6px rgba(38,68,100,.08))}.avatar-choice-label{display:block;font-size:10px;font-weight:800;color:#526b8e;margin-top:6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.avatar-choice-check{position:absolute;top:6px;right:6px;width:20px;height:20px;border-radius:50%;background:#1479ff;color:#fff;display:grid;place-items:center;box-shadow:0 3px 8px rgba(20,121,255,.25)}
   .colour-choice-list{display:grid;grid-template-columns:1fr 1fr;gap:9px}.colour-choice-row{display:flex;flex-direction:column;align-items:flex-start;gap:9px;padding:12px;border:1px solid #e0eaf5;border-radius:16px;background:#fbfcfe}.colour-choice-row>div:first-child{display:flex;flex-direction:column;gap:2px}.colour-choice-row b{font-size:11px;color:#0d2d63}.colour-choice-row span{font-size:10px;color:#8391a7}.colour-choice-swatches{display:flex;gap:7px;flex-wrap:wrap}.colour-choice-swatch{width:27px;height:27px;border-radius:50%;border:3px solid #fff;box-shadow:0 0 0 1px #dce4ef;transition:.15s}.colour-choice-swatch:hover{transform:scale(1.08)}.colour-choice-swatch.selected{box-shadow:0 0 0 2px #1479ff,0 3px 8px rgba(20,121,255,.16)}
   .avatar-creator-actions{display:flex;gap:9px;padding-top:2px;border-top:1px solid #edf1f6}.avatar-creator-actions>*{flex:1;display:flex;justify-content:center;align-items:center;gap:6px}.avatar-creator-note{font-size:10px;color:#8291a8;text-align:center;margin-top:-8px}
...
function VisualOptions({title,options,selected,makeConfig,onSelect}:{title:string;options:readonly OptionTuple[];selected?:string;makeConfig:(id:string)=>AvatarConfig;onSelect:(id:string)=>void}){
 return <section className="avatar-choice-section"><div className="avatar-choice-title">{title}</div><div className="avatar-choice-grid">{options.map(([id,label])=><button type="button" key={id} className={`avatar-choice ${selected===id?'selected':''}`} onClick={()=>onSelect(id)} aria-label={label}><span className="avatar-choice-art"><Avatar config={makeConfig(id)} size={82}/></span><span className="avatar-choice-label">{label}</span>{selected===id&&<span className="avatar-choice-check"><Check size={13}/></span>}</button>)}</div></section>;
}
...
function ColourOptions({config,update}:{config:AvatarConfig;update:(patch:Partial<AvatarConfig>)=>void}){
 ...
 return <div className="colour-choice-list">{groups.map(group=><section key={group.key} className="colour-choice-row"><div><b>{group.label}</b><span>Choose a colour</span></div><div className="colour-choice-swatches">{group.values.map(value=><button type="button" key={value} className={`colour-choice-swatch ${config[group.key]===value?'selected':''}`} style={{background:value}} onClick={()=>update({[group.key]:value})} aria-label={`${group.label}: ${value}`}/>)}</div></section>)}</div>;
}
