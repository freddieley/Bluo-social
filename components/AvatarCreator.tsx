...
export function AvatarCreator({config,setConfig,close}:{config:AvatarConfig;setConfig:(c:AvatarConfig)=>void;close:()=>void}){
 ...
 return <div className="avatar-creator">
  <style jsx>{`
   .avatar-creator{font-family:'DM Sans',system-ui,sans-serif;color:#102f67;margin:-18px -20px -28px;background:#fff;min-height:calc(100dvh - 10px);display:flex;flex-direction:column;overflow:hidden}
   .ac-header{padding:4px 22px 12px}.ac-header-row{display:flex;align-items:center;gap:12px}.ac-back{width:36px;height:36px;border-radius:12px;background:#eef5ff;color:#1479ff;display:grid;place-items:center}.ac-title{font-family:'Plus Jakarta Sans';font-size:25px;line-height:1.1;letter-spacing:-.8px;font-weight:700;margin:0}.ac-sub{font-size:12px;color:#7286a6;margin:5px 0 0 48px;line-height:1.45}
   .ac-preview{margin:0 22px 14px;border-radius:25px;background:linear-gradient(145deg,#eaf5ff 0%,#f8fbff 55%,#fff 100%);border:1px solid #dceaf8;min-height:188px;display:grid;place-items:center;position:relative;overflow:hidden;box-shadow:0 10px 28px rgba(29,91,163,.08)}
   ...
   .ac-tabs{display:flex;gap:6px;padding:4px 22px 11px;overflow-x:auto;scrollbar-width:none}.ac-tabs::-webkit-scrollbar{display:none}.ac-tab{white-space:nowrap;padding:8px 13px;border-radius:999px;background:#f1f5fa;color:#7084a3;font-size:10px;font-weight:800;border:1px solid transparent}.ac-tab.active{background:#1479ff;color:#fff;box-shadow:0 5px 12px rgba(20,121,255,.18)}
   .ac-scroll{flex:1;overflow:auto;padding:0 22px 120px;-webkit-overflow-scrolling:touch}.ac-section-title{font-family:'Plus Jakarta Sans';font-size:16px;font-weight:700;letter-spacing:-.25px;margin:2px 0 10px}.ac-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.ac-choice{position:relative;min-width:0;padding:7px 6px 9px;border:1px solid #e0e9f4;background:#f8fbff;border-radius:20px;text-align:center;transition:.16s;box-shadow:0 3px 10px rgba(30,70,115,.035)}.ac-choice:active{transform:scale(.98)}.ac-choice.selected{border-color:#1479ff;background:#edf6ff;box-shadow:0 0 0 2px rgba(20,121,255,.13),0 7px 18px rgba(20,121,255,.10)}.ac-art{height:105px;border-radius:16px;background:linear-gradient(145deg,#f2efff,#edf6ff);display:grid;place-items:center;overflow:hidden}.ac-art :global(svg){width:96px!important;height:96px!important}.ac-label{display:block;color:#1b3b70;font-size:10px;font-weight:800;margin-top:7px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ac-check{position:absolute;top:10px;right:10px;width:22px;height:22px;border-radius:50%;background:#1479ff;color:#fff;display:grid;place-items:center;box-shadow:0 4px 10px rgba(20,121,255,.22)}
   .ac-colours{display:grid;grid-template-columns:1fr 1fr;gap:10px}.ac-colour{padding:13px;border:1px solid #e0e9f4;background:#f8fbff;border-radius:18px}.ac-colour b{display:block;font-size:11px}.ac-colour span{display:block;color:#8291a8;font-size:9px;margin:2px 0 10px}.ac-swatches{display:flex;gap:8px;flex-wrap:wrap}.ac-swatch{width:29px;height:29px;border-radius:50%;border:3px solid #fff;box-shadow:0 0 0 1px #d9e3ef}.ac-swatch.selected{box-shadow:0 0 0 2px #1479ff,0 4px 10px rgba(20,121,255,.14)}
   .ac-footer{position:sticky;bottom:0;display:flex;gap:10px;padding:12px 22px 18px;background:linear-gradient(to bottom,rgba(255,255,255,0),#fff 22%);z-index:4}.ac-footer button{flex:1;height:56px;border-radius:19px;font-weight:800;font-size:15px}.ac-done{background:#edf3fa;color:#183764}.ac-save{background:#1479ff;color:#fff;box-shadow:0 10px 24px rgba(20,121,255,.24);display:flex;align-items:center;justify-content:center;gap:7px}
   @media(min-width:700px){.avatar-creator{margin:-18px -20px -28px;min-height:0}.ac-preview{margin:0 28px 16px;min-height:220px}.ac-header{padding-left:28px;padding-right:28px}.ac-tabs,.ac-scroll{padding-left:28px;padding-right:28px}.ac-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.ac-footer{padding-left:28px;padding-right:28px}.ac-art{height:115px}}
  ...
  `}</style>
  <header className="ac-header"><div className="ac-header-row"><button className="ac-back" onClick={close} aria-label="Close"><ChevronLeft size={20}/></button><h2 className="ac-title">Make your Bluo</h2></div><p className="ac-sub">Create a character that feels like you. You can change it whenever you want.</p></header>
  ...
  <div className="ac-tabs">{tabs.map(t=><button key={t.id} className={`ac-tab ${tab===t.id?'active':''}`} onClick={()=>setTab(t.id)}>{t.label}</button>)}</div>
  ...
  <div className="ac-footer"><button className="ac-done" onClick={close}>Done</button><button className="ac-save" onClick={close}><Check size={18}/>Save character</button></div>
 </div>;
}
function Options({title,options,selected,patch,update}:{title:string;options:Opt;selected?:string;patch:(id:string)=>Partial<AvatarConfig>;update:(p:Partial<AvatarConfig>)=>void}){
 return <section><h3 className="ac-section-title">{title}</h3><div className="ac-grid">{options.map(([id,label])=><button key={id} className={`ac-choice ${selected===id?'selected':''}`} onClick={()=>update(patch(id))}><span className="ac-art"><Avatar config={{...({} as AvatarConfig),...({skin:'#D99A70',hair:'#2A1C17',shirt:'#1479FF',bg:'#EAF4FF',eyes:'#25364d',mouth:'#8f4e51'}),...patch(id)}} size={96}/></span><span className="ac-label">{label}</span>{selected===id&&<span className="ac-check"><Check size={13}/></span>}</button>)}</div></section>;
}
function Colours({config,update}:{config:AvatarConfig;update:(p:Partial<AvatarConfig>)=>void}){
 ...
 return <div className="ac-colours">{groups.map(([label,key,values])=><section className="ac-colour" key={key}><b>{label}</b><span>Choose a colour</span><div className="ac-swatches">{values.map(v=><button key={v} className={`ac-swatch ${config[key]===v?'selected':''}`} style={{background:v}} onClick={()=>update({[key]:v})} aria-label={label}/>)}</div></section>)}</div>;
}
