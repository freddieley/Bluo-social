...
export function MapPanel({ people, filter, onFilter }: { people: Person[]; filter: 'friends' | 'nearby' | 'everyone'; onFilter: (f: 'friends' | 'nearby' | 'everyone') => void }) {
  ...
  return <section className="card map-card">
    <div className="map-surface" style={{ background: '#dcebd9' }}>
      {lowData ? <img src="/psc-campus-map.svg" alt="Peter Symonds College campus map" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} /> : <div ref={mapHost} aria-label="Peter Symonds College street map" style={{ position: 'absolute', inset: 0 }} />}
      {lowData && positioned.map(({ person, position }) => <div key={person.id} style={{ position: 'absolute', left: `${position[0]}%`, top: `${position[1]}%`, transform: 'translate(-50%,-100%)', zIndex: 5 }}><div style={{ width: 40, height: 40, borderRadius: '50%', background: '#1479ff', border: '3px solid #fff', boxShadow: '0 6px 18px rgba(10,70,150,.3)', display: 'grid', placeItems: 'center' }}><Avatar config={person.avatar} size={30} /></div><div style={{ marginTop: 4, background: 'rgba(255,255,255,.95)', padding: '3px 7px', borderRadius: 7, boxShadow: '0 4px 12px rgba(20,50,90,.14)', font: '800 10px system-ui', whiteSpace: 'nowrap', color: '#0d2d63' }}>{person.name}</div></div>)}
      {!lowData && !mapReady && <div className="map-loading"><Navigation size={20} /><b>Loading campus map…</b><span>The saved PSC map is used automatically on slower connections.</span></div>}
    ...
    <div className="map-overlay">
      ...
      <div className="map-info"><span className="mode-dot" style={{ display: 'inline-block' }} /> {lowData ? 'Saved campus map' : 'Live street map'} · {people.length} visible</div>
    </div>
